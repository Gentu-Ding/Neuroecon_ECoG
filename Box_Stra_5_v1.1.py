...
def get_timing():
    ...
    timing = {"InterTrial" : random.uniform(500,500)/speedup,
          "NewRound" : random.uniform(1500,1500)/speedup,
              # "ChoicePresentation" : 2000/speedup,
              "SubjectChoice" : (4000)/speedup,
              "OpponentChoice" : random.uniform(700.0,1300.0)/speedup,
              "CalculatePayoff": 750.0/speedup,
...
def displayImage(filepath, stage="doingInstructions"):
    global stagename, screen, timing
    stagename = stage
    whiteOut()
    map_surface = pygame.image.load(filepath).convert()
...
def instructions():
    ...
    message(["Instructions ( To skip: press 's').",
             "",
             "To next slide: press ENTER;",
             "To previous slide: press LEFT or 1."])

    if keyY==True:
        stagename="doingInstructions"

        #for path in sorted(glob.glob("Instructions_Strong/Box_intr_s/Box_intr_s*.png")):
            #print(path)
            #displayImage(path)
        paths=[]
    ...
    #displayImage("Instructions_Strong/Practice_continue_skip.png", "SkipPractice")
    
    message(["We are moving on to a few practice rounds now.",
             "",
             "Click or Press ENTER to continue",
             "Press 's' to skip"],stage="SkipPractice")
...
def handler(event, wait2timer):
    ...
    global InvestAmount, confirm, keyY,stagename, a, notSkip
    ...
    if event.type == QUIT:
        ...
        if event.key == K_p:
            ...
            return retme
        if event.key in [K_RETURN, K_KP_ENTER, K_SPACE, K_DOWN, K_RIGHT, K_n] and stagename == "doingInstructions":
                #writeQueue(roundnum,'AcknowledgeInstructions')
                a=1
                return False
        if event.key in [K_LEFT, K_UP, K_b, K_1, K_KP1] and stagename == "doingInstructions":
                #writeQueue(roundnum,'AcknowledgeInstructions')
                a=-1
                return False
            
        if event.key in [K_RETURN, K_KP_ENTER, K_SPACE] and (stagename == "Message" or stagename=="SkipPractice"):
                #writeQueue(roundnum,'AcknowledgeMessage')
                keyY=True
                #notSkip=True
                return False
        ...
        if event.key == K_s and stagename == "Message":
                #writeQueue(roundnum,'AcknowledgeMessage')
                keyY=False
                #notSkip=False
                return False
        if event.key == K_s and stagename == "SkipPractice":
                #writeQueue(roundnum,'AcknowledgeMessage')
        ...
        if event.key in [K_KP0, K_0] and stagename =="SubjectChoice" and canRespond and (not gIsPaused):
                #writeChoice(roundnum,'L')
                InvestAmount = 0
        ...
        if event.key in [K_KP1, K_1] and stagename =="SubjectChoice" and canRespond and (not gIsPaused):
                #writeChoice(roundnum,'L')
                InvestAmount = 1
                drawchoicebars(InvestAmount)
                flipupdate()
        ...
        if event.key in [K_KP2, K_2] and stagename =="SubjectChoice" and canRespond and (not gIsPaused):
                #writeChoice(roundnum,'L')
                InvestAmount = 2
                drawchoicebars(InvestAmount)
                flipupdate()
        ...
        if event.key in [K_KP3, K_3] and stagename =="SubjectChoice" and canRespond and (not gIsPaused):
                #writeChoice(roundnum,'L')
                InvestAmount = 3
                drawchoicebars(InvestAmount)
                flipupdate()
        ...
        if event.key in [K_KP4, K_4] and stagename =="SubjectChoice" and canRespond and (not gIsPaused):
                #writeChoice(roundnum,'L')
                InvestAmount = 4
                drawchoicebars(InvestAmount)
                flipupdate()
        ...
        if event.key in [K_KP5, K_5] and stagename =="SubjectChoice" and canRespond and (not gIsPaused):
                #writeChoice(roundnum,'L')
                InvestAmount = 5
                drawchoicebars(InvestAmount)
                flipupdate()
        ...
        #if event.key in [K_RETURN, K_KP_ENTER, K_SPACE]and stagename =="SubjectChoice" and canRespond and (not gIsPaused):
...
def MainEvent(screen,Data,DataFile,Pay,PayFile, TimeStamp,TimeFile, OpponentFile, StartTime, SID, role,doInstructions,practice):                    
    ...
    if practice==True:
        rounds=3 #Number of Practice Rounds
...
##    BarPosition = (100, 410)  # for bar
    ...
    if practice==0:
        message(["We are now proceeding to the task.",
                 "",
    ...
    flipupdate()
    wait(1.00) # wait 1s before task starts
    ...
    while round <= rounds  :
        ...
        TimeStamp.writerow([round,practice,'NewRound',str(time1),time1-time_end])
        TimeFile.flush()
...
##        bar(Award,screen,BoxSize,(Position3[0]+(BoxSize[0]+1)*EndowmentA,Position3[1]),BackgroundColor, BoxColorGreen,0)
        ...
        TimeStamp.writerow([round,practice,'PresentGame',str(time2),time2-time1])
        TimeFile.flush()
...
##        # change bar into green
        ...
        if confirm==0:
            time_end=time.clock()-StartTime

            TimeStamp.writerow([round,practice,'ChoiceUncaught',str(time_end),time_end-time2])
            TimeFile.flush()
            
            Payoff = 0   
            textD = font.render("You did not decide in time ! " , 1,BoxColorRed,BackgroundColor)
        ...
        TimeStamp.writerow([round,practice,'Buttonpress',str(time3),time3-time2])
        TimeFile.flush()
        ...
        #TimeStamp.writerow([round,practice,'ShowOppoInvest',str(time.clock()-StartTime)])
        #TimeFile.flush()
        #wait(timing["CalculatePayoff"])
        ...
        # write datafile
        time4=time.clock()-StartTime
        print time4-time3
        TimeStamp.writerow([round,practice,'StartFeedback',str(time4),time4-time3])
        TimeFile.flush()
        ...
        #MainLock.acquire()
        time_end=time.clock()-StartTime
        TimeStamp.writerow([round,practice,'EndFeedback',str(time_end),time_end-time4])
        TimeFile.flush() 
...
TimeStamp.writerow(['Round','Practice','Event','Time','Timing'])
TimeFile.flush()
