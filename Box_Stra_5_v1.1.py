...
def handler(event, wait2timer):
    ...
    global InvestAmount, confirm, keyY,stagename, a
...
##    BarPosition = (100, 410)  # for bar
    ...
    while round <= rounds  :
        ...
        flipupdate()
        #waitforspacebar()
        TimeStamp.writerow([round,practice,'NewRound',str(time.clock()-StartTime)])
        TimeFile.flush()
        wait(timing["NewRound"])
...
##        # change bar into green
        ...
        #MainLock.acquire()
        TimeStamp.writerow([round,practice,'Buttonpress',str(time.clock()-StartTime)])
        TimeFile.flush()
        ...
        # write datafile
        TimeStamp.writerow([round,practice,'StartPayoffFeedback',str(time.clock()-StartTime)])
        TimeFile.flush()
